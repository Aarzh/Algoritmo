...
#include <boost/graph/graphviz.hpp> // incluye la función de impresión
#include <stack>
#include <map>
#include <queue>
#include <vector>
...
void AgregaEdge(int s, int d, int w, grafo_direccional_pesado &a);
void QuitarNodo(int cual, grafo_direccional_pesado &a);
void AgregaNodo(grafo_direccional_pesado &grafo);
void QuitarEdge(int s, int d, grafo_direccional_pesado &graf);
void AgregarEdgeSinDireccion(int s, int d, int w, grafo_direccional_pesado &a);
...
int main()
{
    ...
    AgregaEdge(0 ,2, 8, g);
    AgregaEdge(0, 3, 8, g);// función que añade arista pesada al grafo g
    AgregaEdge(2, 1, 7, g);
    AgregaEdge(2, 9, 4, g);
    AgregaEdge(2, 4, 8, g);
    AgregaEdge(1, 4, 7, g);
    ...
    AgregaEdge(9, 2, 10, g);
    AgregaEdge(9, 5, 6, g);
    AgregaEdge(10, 11, 6, g);
    AgregaEdge(11, 8, 2, g);
    AgregaEdge(11, 10, 8, g);
    AgregaEdge(11, 13, 9, g);
    AgregaEdge(12, 13, 6, g);
    AgregaEdge(13,12,12,g);
    //QuitarEdge(12,13,g);
    //AgregaNodo(g); // función que añade vertices nuevos al grafo con un nuevo índice (hace push_back al vector)
    //add_edge(22, 12, 2, g);
    /*QuitarNodo(6, g);
    QuitarNodo(1, g);
    QuitarNodo(2, g);
    ...
    bfs(g, 0);
    dfs(g,0);
    vector<pair<Vertex, int>> Prim = prim(g, 0); //Here we get a vector of pairs, telling us the minimum spanning tree according to prim
    ...
    vector<vector<int>> FloydWarhsall = floydWarshall(g); //He





   //breadth_first_search(graph, a,default_bfs_visitor);
